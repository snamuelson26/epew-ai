-- ============================================================
-- EPEW EDE
-- Meeting 3 / Campaign Authorization Gate
--
-- Adds explicit meeting/campaign authorization fields and makes
-- the atomic allocation function enforce them.
-- ============================================================

alter table public.entrepreneurs
  add column if not exists meeting_1_status text default 'pending',
  add column if not exists meeting_2_status text default 'pending',
  add column if not exists meeting_3_status text default 'pending',
  add column if not exists campaign_authorized_at timestamptz,
  add column if not exists campaign_authorized_by text;
...
begin
  ...
  --
  -- An entrepreneur cannot receive Supporter allocation until
  -- Meetings 1 and 2 are completed, Meeting 3 has reached the
  -- campaign-launch stage, and EPEW has formally authorized the
  -- campaign.
  -- ----------------------------------------------------------

  if coalesce(v_entrepreneur.meeting_1_status, '') <> 'completed' then
    raise exception
      'Entrepreneur % has not completed Meeting 1.',
      p_entrepreneur_id;
  end if;

  if coalesce(v_entrepreneur.meeting_2_status, '') <> 'completed' then
    raise exception
      'Entrepreneur % has not completed Meeting 2.',
      p_entrepreneur_id;
  end if;

  if coalesce(v_entrepreneur.meeting_3_status, '') not in (
    'active',
    'completed'
  ) then
    raise exception
      'Entrepreneur % has not reached the Meeting 3 campaign stage.',
      p_entrepreneur_id;
  end if;

  if coalesce(v_entrepreneur.campaign_status, '') <> 'Campaign Active' then
    raise exception
      'Entrepreneur % does not have an active campaign.',
      p_entrepreneur_id;
  end if;

  if v_entrepreneur.campaign_authorized_at is null then
    raise exception
      'Entrepreneur % does not have formal EPEW campaign authorization.',
      p_entrepreneur_id;
  end if;
