...
function EmanonRegisterContent() {
  ...
  async function submit(event: FormEvent) {
    ...
    window.location.href = `${data.loginUrl ?? (isOrgdh ? "/orgdh/login" : "/emanon/login")}?registered=1`;
  }
  ...
  return <main className="min-h-screen bg-[#f5f7fb] px-4 py-12 text-[#06245c]">
    <div className="mx-auto max-w-xl rounded-3xl bg-white p-8 shadow-2xl">
      <p className="mb-2 text-center text-sm font-bold uppercase tracking-widest text-green-700">{organizationName}</p>
      ...
      {invite && <form onSubmit={submit} className="space-y-5">
        ...
        <button disabled={busy} className="w-full rounded-xl bg-[#06245c] p-4 text-xl font-bold text-white disabled:opacity-60">{busy ? "Creating secure access..." : `Activate ${organizationName} Access`}</button>
      </form>}
    </div>
  </main>;
}
