...
function safeRedirect() {
  ...
  return requested?.startsWith("/emanon/") && !requested.startsWith("//")
    ? requested
...
export default function EmanonLoginPage() {
  ...
  async function submit(event:FormEvent){event.preventDefault();setBusy(true);setMessage("");try{const response=await fetch("/api/emanon/login",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({email:email.trim().toLowerCase(),password,redirectTo:safeRedirect()})});const data=await response.json();if(!response.ok){return setMessage(data.error??"Unable to sign in.");}window.location.href=data.redirectTo??"/emanon/communication-center";}catch{setMessage("The EPEW login service could not be reached. Please check your connection and try again.");}finally{setBusy(false);}}
