...
function messageFor(
  ...
  const firstName = entrepreneurName.trim().split(/\s+/)[0] || entrepreneurName;
...
export default function PotentialSupportersPage() {
  ...
  async function addProspect(event: FormEvent) {
    ...
    if (!profile || !name.trim() || !phone.trim() || !email.trim()) {
      setNotice("Please enter the supporter name, phone number, and email address.");
      return;
    }
    ...
    setRelationship("");
    setNotes("");
    setNotice("Potential supporter added successfully.");
    setSaving(false);
    await loadPage();
  }
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <form onSubmit={addProspect} className="rounded-3xl bg-white p-5 shadow md:p-7">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <label className="font-bold text-slate-700">Phone Number<input required inputMode="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className="mt-2 w-full min-w-0 rounded-xl border border-slate-300 p-3 font-normal" /></label>
              <label className="font-bold text-slate-700">Email Address<input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="mt-2 w-full min-w-0 rounded-xl border border-slate-300 p-3 font-normal" /></label>
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              {contacts.map((contact) => (
                <article key={contact.id} className="min-w-0 rounded-2xl border border-slate-200 p-4">
                  ...
                  <p className="mt-3 break-all text-sm text-slate-600">{contact.phone}</p>
                  <p className="break-all text-sm text-slate-600">{contact.email}</p>
