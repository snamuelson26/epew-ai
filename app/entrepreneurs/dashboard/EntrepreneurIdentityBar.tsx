...
export default function EntrepreneurIdentityBar() {
  ...
  async function loadIdentity() {
    ...
    try {
      ...
      if (Number.isInteger(requestedId) && requestedId > 0) {
        applicationQuery = applicationQuery.eq("id", requestedId);
      } else {
  ...
  const accountNumber =
    ...
    (Number(application?.id) === 27 ? "FFR-001" : "Pending");
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-4 px-6 py-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-center gap-4">
          <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-full border-2 border-green-600 bg-slate-100">
            {entrepreneurPhoto ? (
              <img src={entrepreneurPhoto} alt={entrepreneurName} className="h-full w-full object-cover" />
            ) : (
        ...
        <div className="flex items-center gap-4 lg:mr-48 xl:mr-56">
          <div className="flex h-16 min-w-24 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-slate-50 px-3">
            {businessLogo ? (
              <img src={businessLogo} alt={`${businessName} logo`} className="max-h-14 max-w-32 object-contain" />
            ) : (
              <span className="text-center text-xs font-bold text-slate-500">Business Logo<br />Pending</span>
            )}
          </div>

          <button type="button" onClick={handleLogout} className="rounded-xl bg-red-600 px-5 py-3 font-extrabold text-white shadow transition hover:bg-red-700">
            Logout
          </button>
        </div>
      </div>
